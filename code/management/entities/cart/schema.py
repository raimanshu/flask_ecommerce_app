...
class CartBase(DefaultEntityBase):
    cart_id: str = Field(
...
class AdditionalValidators:
    # @validator("cartname")
    # def validate_cartname(cls, value):
    #     if not value or len(value) < 3:
    #         raise ValueError("Cartname must be at least 3 characters long.")
    #     return value
...
class CartCreate(AdditionalValidators, CartBase):

    @validator("cart_id", pre=True, always=True)
    @classmethod
    def generate_cart_id(cls, v):
        if v:
            return str(uuid.uuid4())
        return v


class CartUpdate(AdditionalValidators, CartBase):
...
class CartDelete(CartBase):  # pylint: disable=too-few-public-methods
    name: Optional[str] = None
