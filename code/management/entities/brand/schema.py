...
class BrandBase(DefaultEntityBase):
    brand_id: str = Field(
...
class AdditionalValidators:
    # @validator("brandname")
    # def validate_brandname(cls, value):
    #     if not value or len(value) < 3:
    #         raise ValueError("Brandname must be at least 3 characters long.")
    #     return value
...
class BrandCreate(AdditionalValidators, BrandBase):

    @validator("brand_id", pre=True, always=True)
    @classmethod
    def generate_brand_id(cls, v):
        if v:
            return str(uuid.uuid4())
        return v


class BrandUpdate(AdditionalValidators, BrandBase):
...
class BrandDelete(BrandBase):  # pylint: disable=too-few-public-methods
    name: Optional[str] = None
